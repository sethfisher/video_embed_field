 ...
 */
class VideoEmbedStyleListController extends ConfigEntityListController implements EntityControllerInterface {
   ...
   * {@inheritdoc}
   */
  public function getOperations2(EntityInterface $entity) {
    $flush = array(
      'title' => t('Flush'),
      'weight' => 200,
  ...
  public function render() {
    ...
    $build['#empty'] = $this->t('There are currently no styles. <a href="!url">Add a new one</a>.', array(
      '!url' => $this->urlGenerator->generateFromPath('admin/config/media/video-embed-styles/add'),
    ));
    return $build;
  }
